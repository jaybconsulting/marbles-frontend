...
import { getNextPlayer, players } from '../utils/GameSetup'
import { Box, Grid } from "@chakra-ui/react";
import CardComponent from "./CardComponent";
import { numRows, numCols, boardSpaceSize, boardGapSize } from '../utils/constants';
import woodBackground from '../assets/light-wood-background.webp';
...
export default function Board({
  ...
  for (let row = 0; row < numRows; row++) {
    ...
    if (spaceMarble) {
      spaces.push(
        <MarbleSpace 
          key={`${row}-${col}`} 
          color={`${spaceMarble.colour}-marble`} 
          disabled={(spaceMarble.whereCanMove.length === 0 || disableOtherMarbles) && !spaceMarble.isInPathOfOpposingTeams7} 
          onClick={() => onMarbleClick(spaceMarble)}
        />
      );
    }
    else if (spaceType) {
      spaces.push(
        <Space 
          ...
          number={spaceNumber} 
          colour={spaceColour} 
          disabled={!spaceAvailableForMarble} 
          onClick={() => onBoardPositionClick([row, col])}
        /> 
      );
    }
    else {
  ...
  // How can I replace a portion of a grid with an image? 
  return (
    <Box 
      position="relative"
    >
      <Grid
        templateColumns={`repeat(${numCols}, ${boardSpaceSize}px)`}
        templateRows={`repeat(${numRows}, ${boardSpaceSize}px)`}
        gap={boardGapSize}
        height={`${numRows * boardSpaceSize + (numRows - 1) * boardGapSize}px`}
        width={`${numCols * boardSpaceSize + (numCols - 1) * boardGapSize}px`}
        borderRadius="50%"
        border="1px solid black"
        padding={`${boardSpaceSize * 3}px`}
      ...
      {lastCardPlayed &&
        ...
        >
          <CardComponent
            ...
            currTurn={true}
            playable={true}
            width={boardSpaceSize * 3 + boardGapSize * 2 + 3}
            style={{
              marginLeft: 2,
            }}
          />
